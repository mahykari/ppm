...
for comb in {10,30,100}_{16,32}; do
  IFS='_' read -r n w <<< "$comb"
  printf "I: parameters ndoors=%3d, wordlen=%2d\n" $n $w

  # for s in {128,256,768,1024,1536,2048}; do
    ...
    ./Monitor -protocol yao -security $s -mslen $mslen -sslen $sslen \
      -spec timekeeper.v -wordlen $w -ndoors $n \
      > ./logs/monitor_$n-$w-$s.log 2>&1 &
    mpid=$!
    ./System -protocol yao -security $s -mslen $mslen -sslen $sslen \
